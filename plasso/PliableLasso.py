...
class PliableLasso(BaseEstimator, RegressorMixin):
    def __init__(self, lam=1.0, alpha=0.5, max_iter=1000):
    ...
    def fit(self, X, Z, y):
        ...
        self.beta, self.theta = np.zeros(0), np.zeros((p, k))
        ...
        for i in range(self.max_iter):
            ...
            for pi in range(p):
                ...
                cond_17a = abs(x_j_t_dot_r_minus_j_over_n) <= (1 - alpha) * lam
                cond_17b = soft_thres(v2a(compute_w_j(X, Z, pi)).T @ r_minus_j / n, alpha * lam)
                ...
                if cond_17a <= (1 - alpha) * lam and cond_17b <= 2 * (1 - alpha) * lam:
                    ...
                    cond_19 = la.norm(
                        soft_thres(v2a(compute_w_j(X, Z, pi)).T @ (r_minus_j - X[:, pi] * beta_j_hat), alpha * lam),
                        2
                    )
